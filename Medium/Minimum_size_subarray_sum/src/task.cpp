...
public:
    ...
    int sliding_window(int target, const vector<int>& nums) {
        ...
        for(int i=1;i<(int)nums.size();i++) {
            if(current_sum+nums[i]<target) {
                ...
                while(current_sum+nums[i]-nums[left_index]>=target) {
                ...
                if(current_sum+nums[i]>=target) {
                    current_sum=current_sum+nums[i];
                    right_index++;
                    if(right_index-left_index+1<minimal_length) {
                        minimal_length=right_index-left_index+1;
                    }
                }

            }
        }
        if(minimal_length==INT32_MAX) {
            return 0;
        }
...
int main() {
    ...
    nums={1,4,4};
    assert(s.minSubArrayLen(4,nums) == 1);

    nums={1,1,1,1,1,1,1,1};
    assert(s.minSubArrayLen(11,nums) == 0);

    nums={10,2,3};
    assert(s.minSubArrayLen(6,nums) == 1);
   ...
   // int last_index=0;
    ...
    for(int i=0;i<=(int)nums.size()-132;i++) {
        int tmp_sum=0;

        for(int z=i;z<i+132;z++) {
            tmp_sum+=nums[z];

        }
     //   std::cout<<"first index is " <<i <<" and last is " <<i+131<<std::endl;

        if(tmp_sum>=697439 && tmp_sum<best_sum) {
          //  std::cout<<"ocekavane hranice " <<i <<" and " <<i+131<<std::endl;
          //  first_index=i;
           // last_index=i+131;

        }
    }
   // std::cout<<"ocekavane hranice " <<first_index <<" and " <<last_index<<std::endl;
    //std::cout <<"size is " <<nums.size() <<std::endl;
    assert(s.minSubArrayLen(697439,nums)==132);
